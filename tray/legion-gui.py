...
class HomePage(QWidget):
    ...
    def _build(self):
        ...
        # G-Sync toggle
        gsync_tog = ToggleSwitch(path=GSYNC, read_val=rdsys(GSYNC,"0"))
        gl.addWidget(_setting_row("🔄", "G-Sync",
            "NVIDIA G-Sync variable refresh rate (requires restart).", gsync_tog))
        gl.addWidget(make_div())
        ...
        self.ai_badge = AIBadge(on_change=self._on_ai_engine)
    ...
    def _on_gpu_mode_combo(self, idx):
        ...
        descs    = [
            ...
            "Best performance — NVIDIA RTX 3060 drives everything, more power draw.",
    ...
    def _on_ai_engine(self, val):
        self._ai_on = val
        set_ai_engine(val)
        send_notif("L1 AI Engine " + ("Enabled" if val else "Disabled"),
                   "EPP set to balance_performance" if val else "EPP restored to profile default")
    ...
    def refresh(self, d=None):
        ...
        # AI Engine badge sync — only update from native sysfs; never auto-follow EPP
        ai = d.get("ai_engine")
        if ai is not None:
            # Native sysfs available — mirror it (but only if user hasn't changed it this session)
            self.ai_badge.set_state(ai == "1")
        # If no native path: do NOT touch the badge — keep whatever the user last set
...
class DisplayPage(QWidget):
    ...
    def _build(self):
        ...
        dl.addWidget(NotifyToggle("G-Sync",
