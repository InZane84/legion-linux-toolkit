...
if [[ "$BRAND" == "Legion" || "$BRAND" == "LOQ" ]]; then
    if ! lsmod 2>/dev/null | grep -q "lenovo_legion"; then
        ...
        # Load the module now that it's installed
    ...
    # Only force-load if dmesg shows the ACPI probe failure (e.g. Legion Slim 5 16APH8)
    if dmesg 2>/dev/null | grep -q "probe with driver legion failed\|Could not init ACPI access"; then
        warn "ACPI probe failed — applying force-load fix for this model…"
        ...
        modprobe legion_laptop force=1 2>/dev/null \
            && ok "Force-load applied — update your BIOS for a permanent fix" \
            || warn "Force-load failed — update BIOS at pcsupport.lenovo.com"
    fi
fi
